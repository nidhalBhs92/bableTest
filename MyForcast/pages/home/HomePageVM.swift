...
class HomePageVM {
    ...
    func addCityToList(city: ResultCity) {
        guard let area = city.areaName.first,
              let country = city.country.first else {
                  return
              }
        let lon: Float = Float(city.longitude) ?? 0
        let lat: Float = Float(city.latitude) ?? 0
        self.model.selectedCities.append(.init(id: generateCityId(),
                                               name: area.value,
                                               subName: country.value,
                                               lon: lon,
                                               lat: lat))
    }
